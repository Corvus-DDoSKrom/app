...
(function (root, factory) {
  ...
  const defaultConfig = {
    ...
    labels: {
      ...
      perPage: '{select} entradas', // per-page dropdown label
      noRows: 'No se encontraron entradas', // Message shown when there are no search results
  ...
  const isJson = function (str) {
    ...
    return !(t === null || (!isArray(t) && !isObject(t))) && t
  }
   ...
   * @return {Object}
   */
  const extend = function (src, props) {
   ...
   * @return {Void}
   */
  const each = function (arr, fn, scope) {
    let n
    if (isObject(arr)) {
      for (n in arr) {
   ...
   * @return {Object}
   */
  const createElement = function (a, b) {
    const d = doc.createElement(a)
  ...
  Columns.prototype.order = function (columns) {
    let a; let b; let c; let d; let h; let s; let cell
    const temp = [
      [],
      [],
      [],
  ...
  Columns.prototype.add = function (data) {
    const that = this
    let td; const th = document.createElement('th')
  ...
  Columns.prototype.sort = function (column, direction, init) {
    ...
    let a = 0
    let n = 0
    const th = dt.activeHeadings[column]

    column = th.originalCellIndex

    each(rows, function (tr) {
      const cell = tr.cells[column]
      const content = cell.hasAttribute('data-content') ? cell.getAttribute('data-content') : cell.data
  ...
  Columns.prototype.rebuild = function () {
    let a; let b; let c; let d; const dt = this.dt
    const temp = []
  ...
  Rows.prototype.build = function (row) {
    let td; const tr = createElement('tr')
  ...
  proto.render = function (type) {
    ...
    if (o.ajax) {
      ...
      const xhrProgress = function (e) {
        that.emit('datatable.ajax.progress', e, xhr)
      }

      const xhrLoad = function (e) {
        if (xhr.readyState === 4) {
      ...
      const xhrFailed = function (e) {
        that.emit('datatable.ajax.error', e, xhr)
      }

      const xhrCancelled = function (e) {
  ...
  proto.bindEvents = function () {
    ...
    // Per page selector
    if (o.perPageSelect) {
      const selector = that.wrapper.querySelector('.dataTable-selector')
      if (selector) {
        // Change per page
  ...
  proto.search = function (query) {
    ...
    each(this.data, function (row, idx) {
      const inArray = this.searchData.indexOf(row) > -1

      // https://github.com/Mobius1/Vanilla-DataTables/issues/12
      const doesQueryMatch = query.split(' ').reduce(function (bool, word) {
        let includes = false
        let cell = null
        let content = null
    ...
    if (isObject(data)) {
      ...
      each(data, function (row) {
        ...
        each(row, function (cell, heading) {
          const index = that.labels.indexOf(heading)
